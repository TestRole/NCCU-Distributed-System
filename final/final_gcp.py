...
path = "gs://dataproc-8374220d-c2fc-4779-8bfe-8d3461631a17-asia-northeast1"
...
#-------------------------Load CSV
lines = spark.read.text(path + "/gdelt/gdelt-0000000000*.csv").rdd.map(lambda r: r[0]).filter(lambda x: not "Actor1CountryCode" in x)
...
avgNM = NumMen.reduceByKey(add).join(NMCount).map(lambda x: (x[0], x[1][0]/x[1][1]))
# TotalNM = NumMen.reduceByKey(lambda x,y: (int(x)+int(y)))

#-------------------------MERGE the page rank and NMCount As (Country, pagerank, NMCount)
Merged = ranks.join(avgNM).map(lambda x: (x[0], x[1][0], x[1][1]))
...
#-------------------------SaveAsFile
Merged.repartition(1).saveAsTextFile(path + "/MergedOutPut_avg")
